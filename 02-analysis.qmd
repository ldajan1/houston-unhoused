...
@ NOTE: which ones of these do we want to classify as civility ordinances? All the ones with specific times?
...
```{r}
unhoused_jail <- unhoused |> 
  mutate(judgment = str_to_upper(str_trim(judgment)),
         defendant_name = str_to_upper(str_trim(defendant_name))#,
...
ggplot(shakesnider_jailed, 
       aes(x = judgment_date, y = jail_times, fill = violation_code)) +
